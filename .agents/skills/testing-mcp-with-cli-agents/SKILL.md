...
agents at a rampa checkout and driving them. It is adapted from a
server-agnostic template: `rampa` is the registered server slug, and rampa's
backend isolation is a scratch load-test target plus its process-local run
registry (see below) — not a single scratch-socket env var.
...
2. **The server's backend / side effects.** Keep every tool call away from real
   state, so you can assert against a scratch backend as independent ground truth.

rampa is a load-testing framework, so its side effects are twofold — and both
isolate cleanly without a dedicated scratch-backend env var:

| rampa facet | Scratch-backend lever | Ground-truth check |
|---|---|---|
| run registry (active + completed runs) | process-local and in-memory — a fresh `rampa-mcp` subprocess starts with an empty registry, so isolation is automatic; never reuse one server process across independent tests | the `list_runs` / `get_run` MCP tools (or `get_registry()` in-process) |
| the load it generates (real HTTP/WS/gRPC traffic) | point the test *script* at a scratch/local target — a disposable echo or mock endpoint — never a production system under test | the scratch target's own request log, plus the run's reported metrics via the `metrics` tools |

rampa exposes no single "scratch socket / data dir" env var the way some servers
do; you isolate by (a) letting each `rampa-mcp` subprocess own its registry and
(b) feeding it a scratch script that hits a throwaway target. For reference, the
same two-thing principle across other server kinds:

| Server kind | Scratch-backend lever | Ground-truth check |
|---|---|---|
| tmux control | `LIBTMUX_SOCKET=<scratch>` → an isolated `tmux -L <scratch>` server | `tmux -L <scratch> list-windows` |
| search / index | a scratch index/store dir via the server's data-dir env/flag | inspect the scratch index, not the real store |
...
$ tmux -L cli-harness send-keys -t agent 'cd /repo && <cli launch with a scratch target script>' Enter
$ tmux -L cli-harness capture-pane -p -t agent | tail -5      # poll until the prompt renders
$ tmux -L cli-harness send-keys -t agent 'Use the rampa MCP to start a short load-test run against a local target'
...
With a long-running tool (for rampa, a load-test run with a long duration):
start it, then while the TUI shows
...
$ uv run scripts/mcp_swap.py doctor --server rampa            # effective environment + footguns
$ uv run scripts/mcp_swap.py status --server rampa
$ uv run scripts/mcp_swap.py use-local --server rampa --env KEY=VALUE --dry-run
$ uv run scripts/mcp_swap.py use-local --server rampa --env KEY=VALUE
...
auth-overriding env vars like `OPENAI_API_KEY`. Use `--env` to write any env
into the swapped entry at swap time; rampa has no single scratch-backend var, so
this is mainly for servers that expose one (an isolated socket or data dir).
