...
invocation. The registered server slug here is `rampa` (launched as
`uv --directory <repo> run rampa-mcp`). `<ISOLATION_ENV>` in the command
templates below is a stand-in for a backend-scratching env var — rampa exposes
none (isolate via a scratch load-test target script plus its process-local run
registry instead), so drop the `env`/`-e` fragment for rampa and keep it only
for servers that do expose such a var (e.g. `LIBTMUX_SOCKET` for a tmux server;
a data-dir var for a search server).
...
| codex | `codex exec` | `CODEX_HOME` throwaway **or** `-c` overrides | `codex mcp get rampa` (parses config, no spawn) | `--dangerously-bypass-approvals-and-sandbox` | yes |
...
| grok | `grok -p` / `--single` | `GROK_HOME` **or** `mcp add --scope project` | `grok mcp doctor rampa --json` (real handshake) | `--permission-mode bypassPermissions` | yes |
...
- **Config-less (leanest):** a home dir with only a symlink to real `auth.json`,
  ...
  `-c 'mcp_servers.rampa.command="uv"' -c 'mcp_servers.rampa.args=["--directory","<repo>","run","rampa-mcp"]'` (append `-c 'mcp_servers.rampa.env.<ISOLATION_ENV>="..."'` only for a server that exposes a scratch var; rampa doesn't).
- **Copy-config:** `cp ~/.codex/config.toml <home>/`; symlink `auth.json`;
  rewrite `[mcp_servers.rampa]`. Downside: **drags in the user's hooks/output
...
- `GROK_HOME=<ws>/.grok grok mcp add rampa [-e <ISOLATION_ENV>=...] -- uv --directory <repo> run rampa-mcp`,
...
- `grok mcp doctor rampa --json` is the **best cheap proof of any CLI** — a
...
- **Hidden `--gemini_dir <path>`** flag (not in `--help`) relocates the entire
  ...
  `<gdir>/config/mcp_config.json` your own `{"mcpServers":{"rampa":{...}}}`.
...
- Project `<ws>/.gemini/settings.json` (`{"mcpServers":{"rampa":{"command","args","env"}}}`)
  ...
  project scope.
- Run: `gemini --skip-trust --allowed-mcp-server-names rampa --approval-mode yolo --output-format json -p '<prompt>'` (verified against gemini 0.52.0).
