...
CLIName = t.Literal["claude", "codex", "cursor", "gemini", "grok", "agy", "opencode", "pi"]
...
# tooling state, distinct from the runtime ``rampa-mcp`` package.
STATE_DIR = _xdg_state_home() / "rampa-dev" / "swap"
...
PI_ADAPTER_DIR = pathlib.Path.home() / ".pi" / "agent" / "npm" / "node_modules" / "pi-mcp-adapter"
    ...
    *,
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict) and isinstance(current, dict):
            nested = _jsonc_next_edit(text, value, (*path, key), ensure_ascii=ensure_ascii)
...
@t.overload
def _server_map(info: CLIInfo, config: t.Any, *, create: t.Literal[True]) -> dict[str, t.Any]: ...
...
def _server_map(info: CLIInfo, config: t.Any, *, create: bool) -> dict[str, t.Any] | None:
...
def resolve_repo_meta(repo: pathlib.Path) -> tuple[str, str]:
    ...
    (``rampa-mcp`` → ``rampa``). This matches the slug existing
...
def _normalize_remote_url(url: str) -> str:
    ...
    return f"{scheme}{sep}{authority.rpartition('@')[2]}{slash}{path}".removesuffix(".git")
...
def _points_at(current: McpServerSpec, target: McpServerSpec, repo: pathlib.Path) -> bool:
...
def _cmd_use_local(args: argparse.Namespace) -> int:
    ...
    for cli in targets:
        ...
        try:
            ...
            cli_spec = dataclasses.replace(spec, env=base_env) if (current or extra_env) else spec
        ...
        try:
            ...
            print(
                f"[{label}] write failed ({exc}){rollback_note}; backup at {backup_path}",
                file=sys.stderr,
            )
            had_error = 1
            continue
    ...
    for cli in targets:
        ...
        for key in cli_keys:
            ...
            try:
                ...
                print(
                    f"[{label}] restored, but recovery state could not be updated: {exc}",
                    file=sys.stderr,
                )
                had_error = 1
                break
            ...
            try:
                ...
                print(f"[{label}] restored; backup cleanup failed: {exc}", file=sys.stderr)
                had_error = 1
...
def _all_server_specs(cli: CLIName, config: t.Any, repo: pathlib.Path) -> dict[str, McpServerSpec]:
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    auth_hits = [(var, cli) for var, cli in AUTH_ENV_VARS.items() if os.environ.get(var)]
    if auth_hits:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pu = sub.add_parser("use-local", help="rewrite configs to run this checkout, or a pull request")
    ...
    pu.add_argument(
        ...
        help=(
            ...
            "Use to inject a scratch/isolation var into the entry without a "
            "manual post-edit."
        ),
    )
    ...
    pd = sub.add_parser("doctor", help="report the effective MCP-swap environment (read-only)")
    pd.add_argument("--repo", default=".", help="repo root (default: .)")
    pd.add_argument("--server", help="MCP server name (default: derived from pyproject.toml)")
